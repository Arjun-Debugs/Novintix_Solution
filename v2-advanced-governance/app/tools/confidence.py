...
def score_match(
    ...
    receipt: Optional[NormalizedRecord]
    ...
    if po is not None or receipt is not None:
        ...
        rate_source = getattr(invoice, "rate_source", "static_fallback") or "static_fallback"
        rate_source_label = "live rate" if rate_source == "live_api" else "static fallback rate"
        
        if amount_delta_pct == 0.0:
            amount_score = 40.0
            reasons.append(f"Amount matches exactly after currency conversion ({rate_source_label}) (40.0/40 pts)")
        else:
            ...
            reasons.append(
                f"Amount differs by {amount_delta_pct:.1f}% after FX conversion ({rate_source_label}). "
